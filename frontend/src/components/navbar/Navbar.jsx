...
const Navbar = () => {
    ...
    return (
        <>
            ...
            {/* Header Wrapper — Refined fixed header with subtle border & translucent surface */}
            <header
                className={`fixed top-0 left-0 right-0 z-[990] w-full transition-all duration-300 ${
                    scrolled
                        ? 'bg-white/90 dark:bg-[#080B14]/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 shadow-sm dark:shadow-none'
                        : 'bg-white/70 dark:bg-[#080B14]/70 backdrop-blur-sm border-b border-slate-200/40 dark:border-slate-800/40'
                }`}
            ...
            >
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                    ...
                    {/* MOBILE QUICK LINKS (< md): Home, Plus, Pricing with small clean SVG Icons */}
                    <nav className="flex md:hidden items-center gap-1 text-xs font-medium">
                        <Link
                            to={user ? "/home" : "/"}
                            className="px-2.5 py-1.5 rounded-md text-slate-700 dark:text-slate-300 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-slate-100 dark:hover:bg-slate-800/50 transition-all flex items-center gap-1.5"
                        >
                            <HomeSvgIcon />
                            <span>Home</span>
                        </Link>
                    ...
                    {/* RIGHT ACTIONS: Dark/Light Toggle + Understated Auth */}
                        ...
                        <div className="md:hidden">
                            {user ? (
                                ...
                                <motion.button
                                    ...
                                    aria-label="Sign In"
                                    className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-purple-600 hover:bg-purple-700 text-white font-medium text-xs transition-colors cursor-pointer"
                                >
                                    <span>Sign In</span>
                                </motion.button>
                            )}
