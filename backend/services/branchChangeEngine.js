const sitData = require('../data/branchChange/sit_2025_26_data.json');

// Canonical mapping for standard and abbreviated branch codes
...
function canonicalizeBranch(code) {
    if (!code) return '';
 ...
 */
function getJoinedDataset(dataset = sitData) {
...
/**
 * Statistical engine for a specific target branch
 */
function getTargetBranchStats(targetBranchInput, dataset = sitData) {
    ...
    // Filter confirmed branch-changes into targetBranch
 ...
 */
function getTransitionStats(fromBranchInput, toBranchInput, dataset = sitData) {
 ...
 */
function compareStudentProfile({
    cgpa,
    ...
    // 1. Backlog check (Regulatory mandate)
    if (hasBacklog) {
        observations.push({
